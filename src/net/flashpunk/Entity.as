package net.flashpunk
{
	...
	public class Entity extends Tweener
	{
		...
		/**
		 * Checks for a collision against an Entity type - by using per-pixel comparison.
		 * This function uses the graphic in each entity and sees if there is an opaque
		 * pixel on top of another opaque pixel.
		 * @param	Entity		The Entity to compare with.
		 * @param	Threshold	This defines the opacity of the points to compare for a positive result.
		 * @param	x			Virtual x position to place this Entity.
		 * @param	y			Virtual y position to place this Entity.
		 * @return	True if one or more opaque pixels overlap, false otherwise.
		 */
		public function hitTest(threshold:int, e:Entity, entityThreshold:int, x:Number, y:Number):Boolean
		{
			return (_graphic && e.graphic && _graphic.getBuffer().hitTest(new Point(x, y), threshold, e.graphic.getBuffer(), new Point(e.x, e.y), entityThreshold));
		}
